package io.github.dracosomething.mtfatedunion.renderer.item;
...
public class GaeBolgRenderer extends BlockEntityWithoutLevelRenderer {
    public static final ResourceLocation caliburn = new ResourceLocation("mahoutsukai", "item/caliburn_model");
    public static final ResourceLocation caliburnTex = new ResourceLocation("mahoutsukai", "item/caliburn");
    ...
    public void renderByItem(ItemStack itemStackIn, ItemDisplayContext transform, PoseStack matrix, MultiBufferSource buffer, int var1, int var2) {
        if (itemStackIn != null && itemStackIn.getItem() instanceof Caliburn) {
            matrix.pushPose();
            RenderBaseItem.render(itemStackIn, matrix, buffer, var1, var2, caliburn, true);
            float offsetx = 0.0F;
            float offsety = 0.0F;
            float offsetz = 0.5F;
            float r = 230.0F;
